(function () {
  ...
  const formatTime = value => value ? new Intl.DateTimeFormat("en-GH", { hour: "numeric", minute: "2-digit" }).format(new Date(`2000-01-01T${value}:00`)) : "To Be Announced";
  ...
  const announcementImageMaxBytes = 5 * 1024 * 1024;
  ...
  function validateAnnouncementImage(file) {
    ...
    if (!file.size || file.size > announcementImageMaxBytes) return "The image must be smaller than 5 MB.";
    return "";
  }
  ...
  function eventCard(item) {
    ...
    const publicStatus = item.status === "published" ? `<span class="status-badge">${item.eventDate >= today ? "Upcoming" : "Past"}</span>` : "";
    return `<article class="hub-card publicity-card event-publicity-card">
      ...
      ${status || publicStatus}<h3>${esc(item.title)}</h3>${item.theme ? `<p class="event-card-theme"><span>Theme</span><strong>${esc(item.theme)}</strong></p>` : ""}${item.speaker ? `<p class="event-card-speaker"><span>Speaker</span><strong>${esc(item.speaker)}</strong></p>` : ""}<p class="event-facts"><b>Time: ${esc(formatTime(item.startTime))}</b><span>${esc(item.venue)}</span></p><p>${esc(item.shortDescription)}</p>
      <div class="card-actions"><a class="hub-btn hub-btn-outline" href="/events/${encodeURIComponent(item.slug)}">View details</a>
      ${item.status === "published" ? `<a class="hub-text-link" href="/events/${encodeURIComponent(item.slug)}/calendar.ics" download>Add to Calendar</a>` : ""}
  ...
  async function announcementDetail(slug) {
    ...
    try {
      ...
      main.innerHTML = `<article class="publicity-detail"><header class="detail-header"><div class="hub-container"><a class="back-link" href="/announcements">← Back to Announcements</a><div class="detail-meta"><span class="hub-badge">${esc(item.category)}</span>${item.urgent ? '<span class="urgent-badge">Important notice</span>' : ""}<time>${formatDate(item.publishedAt)}</time></div><h1>${esc(item.title)}</h1><p>${esc(item.summary)}</p><button class="hub-btn hub-btn-secondary" id="sharePublicity" type="button">Share</button></div></header>
        ...
        <aside class="detail-actions"><h2>Announcement links</h2>${item.externalUrl ? `<a href="${esc(item.externalUrl)}"${item.externalUrl.startsWith("/") ? "" : ' target="_blank" rel="noopener noreferrer"'}>${item.externalUrl.startsWith("/events/") ? "View Event Details" : "Open related link ↗"}</a>` : ""}${item.attachmentUrl ? `<a href="${esc(item.attachmentUrl)}" target="_blank" rel="noopener noreferrer">Open attachment ↗</a>` : ""}${!item.externalUrl && !item.attachmentUrl ? "<p>No additional links or attachments.</p>" : ""}</aside></div></article>`;
      setupShare(item);
  ...
  async function eventDetail(slug) {
    ...
    try {
      ...
      document.title = `${item.title} | SRC Digital Hub`;
      const today = new Date().toISOString().slice(0, 10);
      const publicStatus = item.status === "published" ? (item.eventDate >= today ? "Upcoming" : "Past") : item.status;
      main.innerHTML = `<article class="publicity-detail"><header class="detail-header event-detail-header"><div class="hub-container"><a class="back-link" href="/events">← Back to Events</a><div class="detail-meta"><span class="hub-badge">${esc(item.eventType || item.category)}</span>${item.status === "cancelled" ? '<span class="cancelled-badge">Cancelled</span>' : `<span class="status-badge">${esc(publicStatus)}</span>`}</div><h1>${esc(item.title)}</h1>${item.theme ? `<p class="event-detail-theme"><span>Theme</span>${esc(item.theme)}</p>` : ""}<p>${esc(item.shortDescription)}</p><button class="hub-btn hub-btn-secondary" id="sharePublicity" type="button">Share</button></div></header>
        ...
        <aside class="detail-actions"><h2>Event information</h2><dl><div><dt>Date</dt><dd>${formatDate(item.eventDate)}</dd></div><div><dt>Time</dt><dd>${formatTime(item.startTime)}${item.endTime ? ` – ${formatTime(item.endTime)}` : ""}</dd></div><div><dt>Venue / Platform</dt><dd>${esc(item.venue)}</dd></div>${item.theme ? `<div><dt>Theme</dt><dd>${esc(item.theme)}</dd></div>` : ""}${item.topic ? `<div><dt>Topic</dt><dd>${esc(item.topic)}</dd></div>` : ""}${item.speaker ? `<div><dt>Speaker</dt><dd>${esc(item.speaker)}</dd></div>` : ""}${item.eventType ? `<div><dt>Event type</dt><dd>${esc(item.eventType)}</dd></div>` : ""}<div><dt>Organizer</dt><dd>${esc(item.organizer || "To be confirmed")}</dd></div></dl>${item.relatedAnnouncement ? `<a href="/announcements/${encodeURIComponent(item.relatedAnnouncement.slug)}">Read related announcement</a>` : ""}${item.status === "published" ? `<a class="hub-btn hub-btn-outline" href="/events/${encodeURIComponent(item.slug)}/calendar.ics" download>Add to Calendar</a>` : ""}${item.registrationUrl && item.status === "published" ? `<a class="hub-btn hub-btn-primary" href="${esc(item.registrationUrl)}" target="_blank" rel="noopener noreferrer">Register ↗</a>` : ""}</aside></div></article>`;
      setupShare(item);
      ...
      function renderInlineImages() {
        ...
        inlineHost.querySelectorAll(".inline-image-card").forEach(card => {
          ...
          card.querySelector("[data-inline-file]").addEventListener("change", event => {
            ...
            card.querySelector(".inline-selected-file").textContent = file?.name || "JPG, JPEG, PNG or WEBP; maximum 5 MB.";
