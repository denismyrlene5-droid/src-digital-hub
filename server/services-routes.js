...
function createServicesRouter({ repository, uploadDirectory, requireAnyAdmin, requireFeedbackAdmin, requireLostFoundAdmin, requireBusinessAdmin, audit = () => {} }) {
  ...
  router.post("/feedback", publicSubmitLimit, uploadJson, handle((req, res) => {
  ...
  router.post("/feedback/status", statusLimit, smallJson, handle((req, res) => {
  ...
  router.post("/lost-found", publicSubmitLimit, uploadJson, handle((req, res) => {
    rejectBot(req); let upload;
  ...
  router.post("/businesses", publicSubmitLimit, uploadJson, handle((req, res) => {
    rejectBot(req); let upload;
  ...
  router.use("/admin", requireAnyAdmin, uploadJson);
  ...
  router.delete("/admin/lost-found/:id", requireLostFoundAdmin, handle((req, res) => { const previous=repository.getListingAdmin(req.params.id); repository.deleteListing(req.params.id); removeUpload(previous?.imageToken); audit(req.admin, "lost_found.deleted", "lost_found", req.params.id, "Listing removed"); res.json({ ok: true }); }));
  ...
  router.get("/admin/businesses/:id/logo", requireBusinessAdmin, handle((req, res) => { const business=repository.getBusinessAdmin(req.params.id); if(!business?.logoToken)return res.sendStatus(404); res.setHeader("Cache-Control","private, max-age=300"); res.sendFile(uploads.absolute(business.logoToken)); }));
  ...
  router.put("/admin/businesses/:id", requireBusinessAdmin, handle((req, res) => { const previous=repository.getBusinessAdmin(req.params.id); let logo; try { logo=saveUpload(req.body?.upload,"image"); const business = repository.updateBusiness(req.params.id, req.body, logo); if((logo||req.body?.removeLogo)&&previous?.logoToken)removeUpload(previous.logoToken); audit(req.admin, "business.updated", "business", business.id, `${business.name}: ${business.approvalStatus}`); res.json({ business }); } catch(error) { removeUpload(logo); throw error; } }));
  router.delete("/admin/businesses/:id", requireBusinessAdmin, handle((req, res) => { const previous=repository.getBusinessAdmin(req.params.id); repository.deleteBusiness(req.params.id); removeUpload(previous?.logoToken); audit(req.admin, "business.deleted", "business", req.params.id, "Business removed"); res.json({ ok: true }); }));
  return router;
}
