...
test("PDF nominee import preview and published nominee cards work on mobile",async({page},testInfo)=>{
  ...
  expect(draft).toBeTruthy();const draftRow=page.locator(".admin-table tbody tr").filter({hasText:draft.name}).first();await draftRow.getByRole("button",{name:"Preview flyer"}).click();await expect(page.getByRole("dialog").getByAltText("Private campaign flyer preview")).toBeVisible();await page.getByRole("dialog").getByRole("button",{name:"Close"}).click();await page.request.put(`/api/admin/awards/nominees/${draft.id}`,{data:{name:draft.name,program:draft.program,code:draft.code,categoryId:draft.categoryId,level:draft.level,publicationStatus:"published",active:true}});
