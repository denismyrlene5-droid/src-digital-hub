...
function createContentRouter({ repository, uploadDirectory, requireAnyAdmin, requireContentAdmin, requireContentPublisher, requireSuperAdmin }) {
  ...
  router.put("/admin/media/:id",requireContentAdmin,handle((req,res)=>{let cover;try{cover=uploads.save(req.body.cover,"image");const input={...req.body};if(req.admin.role==="content_editor"){const current=repository.getAlbumAdmin(req.params.id);input.status=current.status;input.featured=current.featured;}const album=repository.updateAlbum(req.params.id,input,cover);log(req,"media.updated","media_album",album.id,`${album.title}: ${album.status}`);res.json({album});}catch(error){uploads.remove(cover);throw error;}}));
  router.delete("/admin/media/:id",requireContentPublisher,handle((req,res)=>{const album=repository.deleteAlbum(req.params.id);uploads.remove(album.coverUrl?.split("/").pop());album.items.forEach(item=>uploads.remove(item.imageUrl?.split("/").pop()));log(req,"media.deleted","media_album",album.id,`Album removed: ${album.title}`);res.json({ok:true});}));
  ...
  router.delete("/admin/media-items/:id",requireContentPublisher,handle((req,res)=>{const item=repository.deleteItem(req.params.id);uploads.remove(item.image_token);log(req,"media.photo_deleted","media_item",req.params.id,"Photo removed");res.json({ok:true});}));
  ...
  router.put("/admin/executives/:id",requireContentAdmin,handle((req,res)=>{let photo;try{photo=uploads.save(req.body.photo,"image");const input={...req.body};if(req.admin.role==="content_editor")input.active=repository.getExecutiveAdmin(req.params.id).active;const executive=repository.updateExecutive(req.params.id,input,photo);log(req,"executive.updated","executive",executive.id,`${executive.fullName}: ${executive.active?"active":"inactive"}`);res.json({executive});}catch(error){uploads.remove(photo);throw error;}}));
