...
test("public frontend retains accessibility and responsive spacing polish", async () => {
  ...
  try {
    ...
    assert.match(home, /\/hub\.css\?v=34/);
    assert.match(awards, /\/hub\.css\?v=34/);
  } finally { await app.close(); }
});
...
test("campaign flyers have exact dimensions, stable category links and public draft protection", async () => {
  ...
  try{
    ...
    assert.equal((await fetch(`${app.base}/api/awards/nominees/${nominee.profileSlug}/flyer?format=square&design=unknown`)).status,400);
