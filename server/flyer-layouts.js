...
function composeFlyer({ design, theme, width, height, item, logo, portrait, qr, title, state, instruction, sans, sansBold, serif, textPath }) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}"><defs><linearGradient id="bg" x2="1" y2="1"><stop stop-color="${theme.start}"/><stop offset="1" stop-color="${theme.end}"/></linearGradient><clipPath id="portrait"><rect x="${p.x}" y="${p.y}" width="${p.w}" height="${p.h}" rx="${p.rx}"/></clipPath></defs>
    ...
    ${textPath(stage, 85, footerY + 58, tall ? 26 : 21, sansBold, theme.accent)}${textPath(instruction, 85, footerY + (tall ? 118 : 100), instructionSize, sansBold, theme.text)}${textPath("uccwisesrc.com", 85, height - 85, 27, sans, theme.soft)}
