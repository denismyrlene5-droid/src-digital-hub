...
function createApp(options = {}) {
  ...
  app.get("/api/awards/nominees/:slug/flyer",async(req,res,next)=>{try{const base=publicBaseUrl||`${req.protocol}://${req.get("host")}`;const flyer=await createNomineeFlyer({db,uploadDirectory,publicDirectory,baseUrl:base,selector:{slug:req.params.slug},format:req.query.format||"status",design:req.query.design,allowDraft:false});res.setHeader("Content-Type","image/png");res.setHeader("Content-Disposition",`${req.query.download==="1"?"attachment":"inline"}; filename="${flyer.filename}"`);res.setHeader("Cache-Control","no-store");res.send(flyer.buffer);}catch(error){next(error);}});
